...
//
#if !defined(B_WriteLog_2019_10_17_11_56_28_mxj)
#define		 B_WriteLog_2019_10_17_11_56_28_mxj
/////////////////////////////////////////////////////////
...
//    }mod;
    ...
    std::map< std::string, std::ofstream >      m_mpModInfo;                        //模块信息
...
public:
    ...
    bool        Init(const std::string sFilePath, int iLevel, int iExpireDays, const std::string sDB, int iBufferSize);                                     //初始化日志操作类,设置日志存放路径，记录等级，过期时间，数据库连接字符串，缓存大小
    void		WriteLog(const std::string sRemark, const std::string sModName, unsigned int iLevel);			    //写日志接口函数-字符串
    void		WriteLog(const char* pRemark, unsigned int nLen, const std::string sModName, unsigned int iLevel);				//按十六进制写
